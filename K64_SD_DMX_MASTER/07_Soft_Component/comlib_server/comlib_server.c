...
uint_8 WriteWhichFile;
struct FileInfo fileinfoslave;
...
//			printf("[ServerWriteSDHC]: Old File open correct!\r\n");
		...
		if(sd_fnum == 0)
		{
			if((ptcmd->pcmdbuf[7]<<8|ptcmd->pcmdbuf[8])==0)
			{
			...
			else if((ptcmd->pcmdbuf[7]<<8|ptcmd->pcmdbuf[8])==1)
			{
				WriteWhichFile=1;

				znFAT_Open_File(&fileinfoslave,"/f0_2CH_1.txt",0,1);
				 //删掉原文件，重新建立一个同名文件
				 if(fileinfoslave.File_Size >= 0)
				 {
					dt1.date=fileinfo1.File_CDate;
					dt1.time=fileinfo1.File_CTime;
					znFAT_Close_File(&fileinfoslave);
					znFAT_Delete_File("/f0_2CH_1.txt");
					znFAT_Create_File(&fileinfoslave,"/f0_2CH_1.txt",&dt1);
					len = znFAT_Open_File(&fileinfoslave,"/f0_2CH_1.txt",0,1);
					/*if(!len)
					{
						ack = REVERSE16(eSERVERACK_ERR_NONE);
						pack = (uint_8 *)&ack;
		...
		else if(WriteWhichFile==1)
		{
			znFAT_Open_File(&fileinfoslave,"/f0_2CH_1.txt",0,1);
			//关中断
			DISABLE_INTERRUPTS;
			//_time_get_ticks(&mqx_tick1);
			znFAT_WriteData(&fileinfoslave, sd_flen, &(ptcmd->pcmdbuf[6]));
			//_time_get_ticks(&mqx_tick2);
			//printf("Write SD time = %d\r\n",mqx_tick2.TICKS[0]-mqx_tick1.TICKS[0]);
			//开中断
			ENABLE_INTERRUPTS;
...
//			if(WriteWhichFile==1)
//			{
				_lwevent_set(&lwevent_group, EVENT_UARTSEND);    //允许执行UART发送任务
			}
