...
uint_8 socket(SOCKET s, uint_8 protocol, uint_16 port, uint_8 flag)
{
      ...
      )
   {
      ...
      if (port != 0) {
         IINCHIP_WRITE( Sn_PORT0(s) ,(uint_8)((port & 0xff00) >> 8));
         IINCHIP_WRITE( Sn_PORT1(s) ,(uint_8)(port & 0x00ff));
      } else {
         local_port++; // if don't set the source port, set local_port number.
         IINCHIP_WRITE(Sn_PORT0(s) ,(uint_8)((local_port & 0xff00) >> 8));
         IINCHIP_WRITE(Sn_PORT1(s) ,(uint_8)(local_port & 0x00ff));
      }
...
@return  1 for success else 0.
*/
uint_8 connect(SOCKET s, uint_8 * addr, uint_16 port)
{
    uint_8 ret;
    if
        (
    ...
    else
    {
        ...
        IINCHIP_WRITE( Sn_DPORT0(s), (uint_8)((port & 0xff00) >> 8));
        IINCHIP_WRITE( Sn_DPORT1(s), (uint_8)(port & 0x00ff));
...
@return  1 for success else 0.
*/
uint_16 send(SOCKET s, const uint_8 * buf, uint_16 len)
{
  uint_8 status=0;
  uint_16 ret=0;
  uint_16 freesize=0;
  ...
  // copy data
  send_data_processing(s, (uint_8 *)buf, ret);
...
*/
uint_16 recv(SOCKET s, uint_8 * buf, uint_16 len)
{
   uint_16 ret=0;
   if ( len > 0 )
   {
...
*/
uint_16 sendto(SOCKET s, const uint_8 * buf, uint_16 len, uint_8 * addr, uint_16 port)
{
   uint_16 ret=0;
...
*/
uint_16 recvfrom(SOCKET s, uint_8 * buf, uint_16 len, uint_8 * addr, uint_16 *port)
{
   uint_8 head[8];
   uint_16 data_len=0;
   uint_16 ptr=0;
...
uint_16 macraw_send( const uint_8 * buf, uint_16 len )
{
   uint_16 ret=0;
   uint_8 sock_num;
   sock_num =0;
   ...
   send_data_processing(sock_num, (uint_8 *)buf, len);
...
uint_16 macraw_recv( uint_8 * buf, uint_16 len )
{
   uint_8 sock_num;
   uint_16 data_len=0;
   uint_16 dummyPort = 0;
   ...
   if ( len > 0 )
   {
      ...
      ptr = (uint_16)((ptr & 0x00ff) << 8) + IINCHIP_READ( Sn_RX_RD1(sock_num) );
      //-- read_data(s, (uint_8 *)ptr, data, len); // read data
      ...
      IINCHIP_READ_RXBUF_BURST(sock_num, ptr, data_len, (uint_8*)(buf));
      ptr += data_len;

      IINCHIP_WRITE(Sn_RX_RD0(sock_num),(uint_8)((ptr & 0xff00) >> 8));
      IINCHIP_WRITE(Sn_RX_RD1(sock_num),(uint_8)(ptr & 0x00ff));
