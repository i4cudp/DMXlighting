...
void sysinit(uint_8 * tx_size, uint_8 * rx_size); // setting tx/rx buf size
...
void setRTR(uint_16 timeout); // set retry duration for data transmission, connection, closing ...
void setRCR(uint_8 retry); // set retry count (above the value, assert timeout interrupt)
void clearIR(uint_8 mask); // clear interrupt
uint_8 getIR( void );
void setSn_MSS(SOCKET s, uint_16 Sn_MSSR); // set maximum segment size
uint_8 getSn_IR(SOCKET s); // get socket interrupt status
uint_8 getSn_SR(SOCKET s); // get socket status
uint_16 getSn_TX_FSR(SOCKET s); // get socket TX free buf size
uint_16 getSn_RX_RSR(SOCKET s); // get socket RX recv buf size
uint_8 getSn_SR(SOCKET s);
void setSn_TTL(SOCKET s, uint_8 ttl);
void send_data_processing(SOCKET s, uint_8 *wizdata, uint_16 len);
void recv_data_processing(SOCKET s, uint_8 *wizdata, uint_16 len);

void setGAR(uint_8 * addr); // set gateway address
void setSUBR(uint_8 * addr); // set subnet mask address
void setSHAR(uint_8 * addr); // set local MAC address
void setSIPR(uint_8 * addr); // set local IP address
