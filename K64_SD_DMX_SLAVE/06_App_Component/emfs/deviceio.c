...
extern uint8_t  sdhc_dat_buffer1[512];
...
// res=MS_Init(&blkSize,&numBlks,inquiryResult);
// if(res) err|=0X01;
//
// res=SD_Ready_Go();
// if(res) err|=0X02;
...
UINT8 znFAT_Device_Read_Sector(UINT32 addr,UINT8 *buffer)
{
 ...
 {
 case 0:
	     while(disk_read(0,buffer,addr,1));
		 break;
 //case 1:
	     //while(Device1_Read_Sector(addr,buffer));
		 //break;
 //case...
...
UINT8 znFAT_Device_Write_Sector(UINT32 addr,UINT8 *buffer) 
{
 ...
 {
  case 0:
	     while(disk_write(0, buffer, addr, 1));
		 break;
  //case 1:
	     //while(Device1_Write_Sector(addr,buffer));
		 //break;
  //case...
...
UINT8 znFAT_Device_Read_nSector(UINT32 nsec,UINT32 addr,UINT8 *buffer)
{
 ...
 switch(Dev_No)
 {
  case 0:
         ...
         {
    	     while(disk_read(0,buffer,addr+i,1));
//			  while(scsi_read_sector(addr+i,buffer));
			  buffer+=512;
         }
         break;
  //case 1:
         ...
         //{
         // while(Device1_Read_Sector(addr+i,buffer));
         // buffer+=512;
         //}
         //break;
 }
...
UINT8 znFAT_Device_Write_nSector(UINT32 nsec,UINT32 addr,UINT8 *buffer)
{
 ...
 switch(Dev_No)
 {
  case 0:
         for(i=0;i<nsec;i++)
         {
    	     while(disk_write(0,buffer,addr+i,1));
             buffer+=512;
         }
		  break;
  //case 1:
         //for(i=0;i<nsec;i++)
         //{
         // while(Device1_Write_Sector(addr+i,buffer));
         // buffer+=512;
         //}
		  //break;
 }
...
UINT8 znFAT_Device_Clear_nSector(UINT32 nsec,UINT32 addr)
{
  ...
  switch(Dev_No)
  {
   case 0:
          for(i=0;i<nsec;i++)
          {
//           while(scsi_write_sector(addr+i,znFAT_Buffer));
             while(disk_write(0,znFAT_Buffer,addr,1));
          }
          break;
   //case 1:
          //for(i=0;i<nsec;i++)
          //{
          // while(Device1_Write_Sector(addr+i,znFAT_Buffer));
          //}
          //break;
  }
