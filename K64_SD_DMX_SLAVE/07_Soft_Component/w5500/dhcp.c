...
uint_8*  SRC_MAC_ADDR   =  EXTERN_DHCP_MAC;    // Local MAC address
uint_8*   GET_SN_MASK    = EXTERN_DHCP_SN;     // Subnet mask received from the DHCP server
uint_8*   GET_GW_IP      = EXTERN_DHCP_GW;     // Gateway ip address received from the DHCP server
uint_8*   GET_DNS_IP     = EXTERN_DHCP_DNS;    // DNS server ip address received from the DHCP server
uint_8*   DHCP_HOST_NAME = EXTERN_DHCP_NAME;   // HOST NAME
uint_8*   GET_SIP        = EXTERN_DHCP_SIP;    // Local ip address received from the DHCP server
uint_8    DHCP_SIP[4] = {0,};      // DNS server ip address is discovered
uint_8    DHCP_REAL_SIP[4] = {0,}; // For extract my DHCP server in a few DHCP servers
uint_8    OLD_SIP[4];        // Previous local ip address received from DHCP server

uint_8  dhcp_state       = STATE_DHCP_READY;          // DHCP client status
uint_8  dhcp_retry_count = 0;    // retry count
uint_8  DHCP_timeout     = 0;        // DHCP Timeout flag
uint_32  dhcp_lease_time;    // Leased time
uint_32  dhcp_time       = 0;
uint_32  next_dhcp_time  = 0;      // DHCP Time 1s
uint_32  dhcp_tick_cnt   = 0;                    // 1ms
uint_8  DHCP_timer;

uint_32  DHCP_XID        = DEFAULT_XID;
uint_8 EXTERN_DHCPBUF[1024];
...
uint_8 parseDHCPMSG(uint_16 length);// Receive the message from DHCP server and parse it.
void  reset_DHCP_time(void);        // Initialize DHCP Timer
void  DHCP_timer_handler(void);     // DHCP Timer handler
uint_8 check_leasedIP(void);         // Check the leased IP address
...
//*/
uint_32 htonl(
	uint_32 hostlong		/**< hostshort  - A 32-bit number in host byte order.  */
	)
{
#if ( SYSTEM_ENDIAN == _ENDIAN_LITTLE_ )
	return REVERSE32(hostlong);
#else
	return hostlong;
#endif
}
...
//*/
uint_16 htons(
	uint_16 hostshort	/**< A 16-bit number in host byte order.  */
	)
{
#if ( SYSTEM_ENDIAN == _ENDIAN_LITTLE_ )
	return REVERSE16(hostshort);
#else
	return hostshort;
#endif
}
...
//*/
uint_32 ntohs(
		uint_16 netshort	/**< netshort - network odering 16bit value */
	)
{
#if ( SYSTEM_ENDIAN == _ENDIAN_LITTLE_ )
...
{
  uint_8 ip[4] = {255,255,255,255};
  uint_16 i=0;
  //the host name modified
  uint_8 host_name[12];
  //*((uint_32*)DHCP_SIP)=0;
  //*((uint_32*)DHCP_REAL_SIP)=0;
  ...
  /* MAGIC_COOKIE */
  pRIPMSG->OPT[i++] = (uint_8)((MAGIC_COOKIE >> 24)& 0xFF);
  pRIPMSG->OPT[i++] = (uint_8)((MAGIC_COOKIE >> 16)& 0xFF);
  pRIPMSG->OPT[i++] = (uint_8)((MAGIC_COOKIE >> 8)& 0xFF);
  pRIPMSG->OPT[i++] = (uint_8)(MAGIC_COOKIE& 0xFF);
...
//  sprintf((char*)host_name,"%.4s-%02X%02X%02X",DEVICE_ID,SRC_MAC_ADDR[3],SRC_MAC_ADDR[4],SRC_MAC_ADDR[5]);
  ...
  pRIPMSG->OPT[i++] = (uint_8)strlen((char*)host_name);
  
  strcpy((char*)(&(pRIPMSG->OPT[i])),(char*)host_name);
 
  
  i+=(uint_16)strlen((char*)host_name);
  ...
  //Delay_ms(800);
  sendto(SOCK_DHCP, (uint_8 *)pRIPMSG, sizeof(RIP_MSG), ip, DHCP_SERVER_PORT);
...
//  sprintf((char*)host_name,(char*)"%.4s-%02X%02X%02X",DEVICE_ID,SRC_MAC_ADDR[3],SRC_MAC_ADDR[4],SRC_MAC_ADDR[5]);
  ...
  //printf("send dhcp request\r\n");
    sendto(SOCK_DHCP, (const uint_8 *)pRIPMSG, sizeof(RIP_MSG), ip, DHCP_SERVER_PORT);
  //if(len==0)
...
*********************************************************************************************************
*/
uint_8 parseDHCPMSG(uint_16 length)
{
  uint_8  svr_addr[6];
  uint_16 svr_port;
  uint_16 len;
  ...
  len = recvfrom(SOCK_DHCP, (uint_8 *)pRIPMSG, length, svr_addr, &svr_port);
  ...
  if( *((uint_32*)DHCP_SIP) != 0x00000000 )
  {
    if( *((uint_32*)DHCP_REAL_SIP) != *((uint_32*)svr_addr) &&
            *((uint_32*)DHCP_SIP) != *((uint_32*)svr_addr) )
    {
#ifdef DHCP_DEBUG		
  ...
  while ( p < e ) 
  {
    ...
    case dhcpIPaddrLeaseTime :
      ...
      //´óÐ¡¶Ë×ª»» !!!
      dhcp_lease_time = ntohl(*((uint_32*)p));
#ifdef DHCP_DEBUG			
    ...
    case dhcpServerIdentifier :
      ...
      printf("DHCP_SIP : %u.%u.%u.%u\r\n", DHCP_SIP[0], DHCP_SIP[1], DHCP_SIP[2], DHCP_SIP[3]);
#endif			
      if( *((uint_32*)DHCP_SIP) == 0 ||
          *((uint_32*)DHCP_REAL_SIP) == *((uint_32*)svr_addr) ||
          *((uint_32*)DHCP_SIP) == *((uint_32*)svr_addr) )
      {
        memcpy(DHCP_SIP,p,4);
...
*********************************************************************************************************
*/
uint_8 check_DHCP_state(SOCKET s)
{
  uint_16 len;
  uint_8  type;
...
uint_8 check_leasedIP(void)
{
  ...
  if ( sendto(SOCK_DHCP, (const uint_8*)"CHECK_IP_CONFLICT", 17, GET_SIP, 5000))
  {
  #ifdef DHCP_DEBUG
    printf(" Conflict>\r\n");
  #endif
...
{
  uint_8 txsize[MAX_SOCK_NUM] = {2,2,2,2,2,2,2,2};
  uint_8 rxsize[MAX_SOCK_NUM] = {2,2,2,2,2,2,2,2};
  //uint_8 ip_broadcast[4]={255,};
  uint_8 ip_0[4]={0,};
  DHCP_XID = 0x12345678;
